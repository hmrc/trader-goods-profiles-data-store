...
import uk.gov.hmrc.tradergoodsprofilesdatastore.models.response.Pagination

import javax.inject.{Inject, Singleton}

@Singleton
class PaginationHelper @Inject() (config: Configuration) {

  val localPageSize: Int     = config.get[Int]("pagination-config.local-page-size")
  val localStartingPage: Int = config.get[Int]("pagination-config.local-starting-page")
  ...
  def buildPagination(sizeOpt: Option[Int], pageOpt: Option[Int], totalRecords: Long): Pagination = {
    ...
    val totalPages           = {
      if (mod == 0 && totalRecords != 0) totalRecordsMinusMod / size
      else (totalRecordsMinusMod / size) + 1
    }.toInt
