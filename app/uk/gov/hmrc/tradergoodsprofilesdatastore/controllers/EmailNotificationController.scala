...
@Singleton()
class EmailNotificationController @Inject() (
  ...
  def create(eori: String, recordId: String): Action[EmailNotificationRequest] =
    identify.async(parse.json[EmailNotificationRequest]) { implicit request =>
      emailNotificationsRepository
        .create(eori, recordId, request.body)
        .map(_ => Ok)
    }
